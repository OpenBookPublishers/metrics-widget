...
export class MeasuresGraph extends React.Component {
  ...
  componentDidMount() {
    fetch(
      "https://metrics.api.openbookpublishers.com/events?aggregation=measure_uri&filter=work_uri:info:doi:10.11647/obp.0020"
    )
      .then((res) => res.json())
      .then((json) => {
        this.setState({
...
export class TimelineChart extends React.Component {
  constructor(props) {
    ...
    this.url = new URL(props.hostname + params + cleanDoi);
    this.state = {
      data: [],
      dataIsLoaded: false,
    };
