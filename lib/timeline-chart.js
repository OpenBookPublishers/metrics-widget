...
export default class TimelineChart extends React.Component {
    constructor(props) {
        super(props);
        const params = 'events?aggregation=measure_uri,year&filter=work_uri:info:doi:';
    ...
    render() {
        ...
        const datasets = data.map((measure) => {
            ...
            const measureData = labels.map((year) => {
                // we search for that year inside the measure's data
                const result = measure.data.filter((y) => y.year == year);
